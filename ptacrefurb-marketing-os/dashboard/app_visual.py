...
# -----------------------------
# Load data from Google Sheets through Apps Script
# -----------------------------
...
if not pipeline.empty and "Stage" in pipeline.columns:
    ...
    pipe_group = (
        p.groupby("Stage", dropna=False)
        .agg(Opportunities=("Company", "count"), Value=("Pipeline Value", "sum"))
        .reset_index()
    )
...
.top {{
    display: grid;
    grid-template-columns: 310px 1fr 145px 130px 150px;
    gap: 8px;
...
.section-title {{
    background: linear-gradient(90deg, {RED}, #710000);
    color: white;
    text-align: center;
    font-weight: 900;
    font-size: 14px;
...
.brand-strip {{
    display: grid;
    grid-template-columns: 46% 1px 1fr;
    gap: 20px;
    align-items: center;
    padding: 28px 25px;
}}
...
.funnel-fill {{
    height: 24px;
    background: linear-gradient(90deg, {RED}, #d56a6a);
    color: white;
    font-size: 12px;
    font-weight: 900;
    text-align: right;
...
.donut {{
    ...
    background:
        ...
        conic-gradient({RED} 0 40%, {DARK} 40% 64%, #888 64% 82%, #d0d0d0 82% 100%);
    border: 1px solid #ddd;
}}
...
<div class="sheet">
  <div class="top">
    ...
    <div class="info-box">
      <div class="info-head">Owner</div>
      <div class="info-val">CP</div>
    </div>
    <div class="info-box">
      <div class="info-head">Role</div>
      <div class="info-val">Marketing Director</div>
    </div>
    <div class="info-box date">
      <div class="info-head">Today's Date</div>
      <div class="info-val">{date.today().strftime("%m/%d/%Y")}</div>
    </div>
  </div>
  ...
  <div class="grid-top">
    <div>
      ...
      <div class="section">
        <div class="brand-strip">
          <div class="brand-logo">PTAC<span>refurb</span></div>
          <div class="divider"></div>
          <div class="brand-copy">We help properties <b>save</b><br>money and extend the life<br>of their PTAC units.</div>
        </div>
      </div>
    </div>

    <div class="section">
      <div class="section-title">Marketing Funnel (This Month)</div>
      <div class="funnel">
    ...
    <div class="section">
      <div class="section-title">Lead Sources (This Month)</div>
  ...
  <div class="section">
    <div class="section-title">Top Content (This Month)</div>
    {table_html(content, ["Post Topic", "Platform", "Impressions", "Clicks", "Engagement Rate"], 4)}
  </div>

  <div class="grid-3">
    <div class="section">
      <div class="section-title">Recent Leads</div>
      {table_html(leads, ["Date", "Company", "Contact Name", "Source", "Status", "Estimated Value"], 5)}
    </div>

    <div class="section">
      <div class="section-title">Pipeline at a Glance</div>
      {table_html(pipe_group, ["Stage", "Opportunities", "Value"], 6)}
    </div>

    <div class="section">
      <div class="section-title">Website Health</div>
      ...
      ]), ["Metric", "This Month", "Last Month", "Change"], 5)}
    </div>
  </div>

  <div class="grid-3">
    <div class="section">
      <div class="section-title">Upcoming Content (Next 7 Days)</div>
      {table_html(calendar, ["Date", "Platform", "Post Topic", "Status"], 4)}
    </div>

    <div class="section">
      <div class="section-title">Upcoming Outreach (Next 7 Days)</div>
      {table_html(outreach, ["Date", "Contact Name", "Company", "Outreach Type", "Status"], 4)}
    </div>

    <div class="section">
      <div class="section-title">Priority Website Tasks</div>
      {table_html(website, ["Task", "Priority", "Status", "Target Date"], 4)}
    </div>
  </div>

  <div class="footer">
    <div>Built with purpose. Driven by results. | PTAC Refurb Marketing OS | Owner: <b>CP</b></div>
    <div><b>PTACrefurb</b></div>
  </div>
</div>
"""

components.html(dashboard, height=1450, scrolling=True)
